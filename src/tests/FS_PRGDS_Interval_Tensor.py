...
class FS_PRGDS_tensor():
    ...
    def sample_h(self):
        ...
        #pmf = torch.einsum('tij, tj -> tij', self.pit * the_tm1)
        # sample h_tkk
        for t in range(self.T):
            for k in range(self.K):
    ...
    @timing_decorator
    def sample_the(self):
        h_tdotk = torch.sum(self.h_tkk, axis=1) # 注意这里有修改，之前忘记对行求和了
        h_tp1_dotk = h_tdotk[1:,:]
        h_tp1_dotk = torch.vstack((h_tp1_dotk, torch.zeros(self.K)))
        ...
        for s in range(self.S):
            for k1 in range(self.K):
                for k2 in range(self.K):
                    ...
                    if self.h_skk[s,k1,k2] == 0:
                        Askk = torch.poisson(cc)
                    else:
                        Askk = sampler.sbch(self.h_skk[s,k1,k2], cc)
                    if Askk < 0:
                        Askk = 0
                    elif Askk > 50:
                        Askk = 50
    ...
    def sample_dpgm(self):
        ...
        self.fs_dpgm_model.phi[self.fs_dpgm_model.phi>10] = self.eps
        self.fs_dpgm_model.lam_kk[self.fs_dpgm_model.lam_kk>10] = self.eps

...
from GS_NBRGDS_Interval_Tensor import GS_NBRGDS_tensor
...
def forecast(model, deep=1):
    ...
    for s in range(2):
        if s == 0 :
            expectation_the = (epsi_the + (tau*(pi @ theta[-1,:].view(-1,1)))/psi) / tau   
        else:
            expectation_the = (epsi_the + (tau*(pi @ texp)/psi)) / tau
...
if __name__ == '__main__':
    params = {
    'data_dir': '/home/HuangRui/PointProcess/NodeGroup/data/icews_tensor_reconstruct_3001010_same_src_tar.npz',
    'K': 70,
    'S': 6,
    'burin': 2,
    'maxiter': 4,
